...
const getAppointmentByProfessional = require("../controllers/Appointments/getAppointmentsByProfessional.js");
...
router.get("/professional/:professionalId", async (req, res) => {
  ...
  const appointments = await getAppointmentByProfessional(professionalId);
  try {
    if (appointments.length === 0) {
      res.status(404).json({
...
router.post("/", postAppointmentValidation, async (req, res) => {
  try {
    const { professionalId, clientId, serviceId, date, hour } = req.body;
